...
static std::set<int32_t> gActiveSensors;
static std::mutex gSensorStateMutex;
static constexpr int32_t kBitsAfterSubHalIndex = 24;

/**
 * Set the subhal index as first byte of sensor handle and return this modified
 * version.
 *
 * @param sensorHandle The sensor handle to modify.
 * @param subHalIndex The index in the hal proxy of the sub hal this sensor
 * belongs to.
 *
...
Return<Result> HalProxy::activate(int32_t sensorHandle, bool enabled) {
    ...
    Result result = getSubHalForSensorHandle(sensorHandle)
                            ->activate(clearSubHalIndex(sensorHandle), enabled);

    if (result == Result::OK) {
        std::lock_guard<std::mutex> lock(gSensorStateMutex);
        if (enabled) {
            gActiveSensors.insert(sensorHandle);
        } else {
            gActiveSensors.erase(sensorHandle);
        }
    }
    return result;
}
...
Return<Result> HalProxy::initialize_2_1(
    ...
    // Create the Event FMQ from the eventQueueDescriptor. Reset the read/write
    // positions.
    auto eventQueue =
    ...
    // Create the Wake Lock FMQ from the wakeLockDescriptor. Reset the read/write
    // positions.
...
Return<Result> HalProxy::initializeCommon(
    ...
    // So that the pending write events queue can be cleared safely and when we
    // start threads again we do not get new events until after initialize resets
    // the subhals.
    disableAllSensors();
...
Return<Result> HalProxy::flush(int32_t sensorHandle) {
    ...
    {
        std::lock_guard<std::mutex> lock(gSensorStateMutex);
        if (gActiveSensors.find(sensorHandle) == gActiveSensors.end()) {
...
Return<Result> HalProxy::injectSensorData(const V1_0::Event& event) {
    ...
    if (mCurrentOperationMode == OperationMode::NORMAL &&
        ...
        ALOGE("An event with type != ADDITIONAL_INFO passed to injectSensorData "
              "while operation"
              " mode was NORMAL.");
        result = Result::BAD_VALUE;
    }
...
Return<void> HalProxy::debug(const hidl_handle& fd, const hidl_vec<hidl_string>& /*args*/) {
    ...
    stream << "  Wakelock timeout reset time: " << msFromNs(now - mWakelockTimeoutResetTime)
           << " ms ago" << std::endl;
    // TODO(b/142969448): Add logging for history of wakelock acquisition per
    // subhal.
...
Return<void> HalProxy::onDynamicSensorsDisconnected(
    ...
    // TODO(b/143302327): Block this call until all pending events are flushed
    // from queue
...
void HalProxy::handlePendingWrites() {
    // TODO(b/143302327): Find a way to optimize locking strategy maybe using two
    ...
    while (mThreadsRun.load()) {
        ...
        if (mThreadsRun.load()) {
            ...
            if (pendingWriteEvents.size() > eventQueueSize) {
                // TODO(b/143302327): Check if this erase operation is too inefficient.
                // It will copy all the events ahead of it down to fill gap off array at
                // front after the erase.
    ...
    if (mPendingWriteEventsQueue.empty()) {
        ...
        if (numToWrite > 0) {
            if (mEventQueue->write(events.data(), numToWrite)) {
                // TODO(b/143302327): While loop if mEventQueue->avaiableToWrite > 0 to
                // possibly fit in more writes immediately
